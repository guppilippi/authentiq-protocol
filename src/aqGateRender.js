...
export function unmountGateDao() {
	const root = document.getElementById(GATE_ROOT_ID);
	if (root) root.remove();
	const styleEl = document.getElementById(GATE_STYLE_ID);
	if (styleEl) styleEl.remove();
	const scriptEl = document.getElementById(GATE_SCRIPT_ID);
	if (scriptEl) scriptEl.remove();
	if (_currentBlobUrl) {
		try { URL.revokeObjectURL(_currentBlobUrl); } catch {}
		_currentBlobUrl = null;
	}
	try { delete window.aqGateInit; } catch { window.aqGateInit = undefined; }
}
