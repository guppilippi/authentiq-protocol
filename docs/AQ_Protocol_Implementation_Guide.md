...
**Megjegyzés a jelen állapothoz**: a jelenleg működő és dokumentált referencia a build-elt `aqBoot.js` és `aqProtocolLoader.js` IIFE bundle (forrásmodulok a `loader/src/` mappában, esbuild pipeline; lásd §1.2). A WEB2 lokális dev szerver (read-only) is része a referencia implementációnak (lásd [WEB2 Server Guide](AQ_WEB2_Server_Guide.md) §1). A WEB2 write szerver (`aqServer.js`) szintén működő referencia implementáció (lásd [WEB2 Server Guide](AQ_WEB2_Server_Guide.md) §2). Minden más (fork mechanizmus, watcher, PWA, cross-device flow) **terv**, és a Plan dokumentumba tartozik, nem ide.
...
- `openTokenId` — opcionális string, a megnyitandó tartalmi DAO azonosítója. Feloldási sorrend: URL param `?token=<value>` → `conf.openTokenId` → null (nincs tartalmi DAO betöltve). Devmode-ban path engedett (kezdő `/`), production-ben tokenId (numerikus string).
...
A host felelőssége:
- a `cidBase` megadása,
- az `openTokenId` megadása (opcionális — megadás nélkül tartalmi DAO nem töltődik),
...
→ Lásd [AQ_WEB2_Server_Guide.md](AQ_WEB2_Server_Guide.md) §1.

---

## 13. WEB2 write szerver — AQS (`aqServer.js`)

→ Lásd [AQ_WEB2_Server_Guide.md](AQ_WEB2_Server_Guide.md) §2.
...
- **Panel** (`z-index: 99999`): dropdown, menüpontok listája.
  - Mindig: **Wallet**
  - devMode: **Publish aqBoot.js**, **Publish Protocol**, **Publish Gate**, **Clear IndexedDB**
  - Production: **Fork DAO**
...
**Flow:**
1. Wallet: `fromRawSeed(seedGetRaw(), 1000)`.
...
### 18.5. Publish aqBoot.js (devMode)

**Flow:**
1. Wallet: `fromRawSeed(seedGetRaw(), 1000)`.
2. `GET /js/aqBoot.js` (no-store fetch).
3. `POST <serverUrl>/aq/asset` → CID.
4. CID vágólapra másolás.
