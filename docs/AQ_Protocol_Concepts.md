...
**Korszak-rögzítés szabályai:**

- Minden DAO rögzíti, melyik korszakban keletkezett — az akkori szorzókat örökre magával viszi.
- Egy korábbi korszakban létrejött DAO szorzója **nem módosul** visszamenőleg.
- Minden jelentős eseménynél (létrehozás, WEB2 publikálás, WEB3 publikálás, fork, audit) rögzül az akkori korszak szorzója.
- A DAO életútja egy **szorzó-sorozat**: az összesített megbízhatóság ezek kombinációja.

Következmény: a genesis-korszakban létrehozott DAO-k magasabb zárt köri szorzóval rendelkeznek — a korai közösség befektetésének értéke, ami később nem reprodukálható.
...
**Entitás** (entity, DAO) — önállóan értelmezhető, megfigyelhető egység. A kontextusgráf csomópontja. Rövidítve: DAO. További szinonimák: node, unit, element, component, object, bogyó.

**DAO** — az entitás rövidítése. Identitás és funkció kategóriákban használt. A tradicionális "Decentralized Autonomous Organization" jelentéstől részben eltér: itt bármilyen önállóan értelmezhető protokoll-szintű egység.

**Identitás DAO** (identity DAO) — egyént vagy közösséget reprezentáló DAO. Nem származtatható, nem fork-olható, szabadsági fokkal rendelkezik.

**Funkció DAO** (function DAO) — szolgáltatást vagy információt reprezentáló DAO. Származtatható, fork-olható, hitelességet épít.

**Reláció** (relation, REL) — két vagy több entitás közötti irányított vagy irányítatlan kapcsolat. A kontextusgráfban él formájában reprezentálva. Szinonimák: kapcsolat, viszony, összefüggés, connection, link.

**Kontextusgráf** (contextgraph, CTXG) — azon entitások és relációk irányított gráfja, amelyek egy adott értelmezési keretben relevánsak. A korábbi "flow" kifejezés általánosított formája. Szinonimák: folyamat, lánc, flow, pipeline, workflow, chain.

**Bogyó** (bean) — korai fogalomalkotás maradványa. A minimális funkcionális egységre utalt (bemenet → feldolgozás → kimenet/hiba). Ma a DAO (entitás) szóval felváltva használatos.

**CID** (Content Identifier) — egy tartalomra mutató azonosító. A WEB3 rétegen valódi tartalom-hash (pl. Swarm hash), a WEB2 rétegen random hash.

**tokenId** — egy funkció DAO referenciáját azonosító szám az aqProtocol szerződésében (PageNFT). A tokenId → CID kötés a chain-en (vagy fejlesztői szerveren) él.

**Root Object** — egy adott pillanatban értelmezhető teljes állapot kitüntetett nézőpontja. Meghatározza a root set-et.

**Root set** — a root object által meghatározott explicit allowlist: mely állapotok, tartalmak és funkcionalitások tekinthetők most létezőnek. Ami nincs benne, az protokoll-szinten nem létezik.

**refs** — egy DAO config-jában a kategorizált, névvel hivatkozható erőforrások (kategóriák: js, css, json, html, others). Egy `refs` levél kétféle lehet: **lokális ref** (saját CID/path) vagy **távoli ref** (`{tokenName}` objektum, amely egy másik funkció DAO-ra mutat). A hívó config `tokens` és `contracts` map-jéből áll össze a feloldáshoz szükséges adat.

**Lokális ref** — `refs` levél objektum formában: `{cid, description}` (saját CID) vagy `{path, description}` (csak devMode). A `description` kötelező.

**Távoli ref** — `refs` levél objektum formában: `{tokenName}`, amely egy másik funkció DAO-ra mutató, futásidejű, egyszintű, név-átívelő hivatkozás. A korábbi külön `imports` szekció ebbe olvadt.

**exports** — egy DAO config-jában a lapos `[{category, name}]` lista, amely megjelöli, mely `refs`-bejegyzések érhetők el más DAO-k számára (fork-öröklés és távoli ref céljaként).

**Fork eredet** — egy forkolt DAO szülőjének azonosítása. A config nem hordozza; kizárólag a publikáláskor számolt merkle-root inputja. (A korábbi `_parent` config-mező megszűnt.)

**Szabadsági fok** (degrees of freedom) — identitás DAO kompromittálhatóságának fordított mértéke. Magas = nehezen kompromittálható.

**Hasznosság** (utility) — funkció DAO mérhető használati metrikája (fork count, távoli ref resolve count, stb.). Objektív, átmeneti jelentőségű.

**Hitelesség** (credibility) — funkció DAO hosszú távú alapértéke, használat és visszaigazolás alapján. Immutable: ami egyszer hiteles volt, az marad.

**Bizonyíték** (attestation) — aláírt állítás, amelyet egy DAO más DAO(k)ra vonatkozóan ad ki (pl. "ezt az identitás DAO-t a következő tényezőkkel hoztuk létre"). Értéke az aláíró hitelességével súlyozódik.

**Ephemeral fork** — rövid életű származtatott DAO, amely egyetlen publikálási műveletre szolgál. A publikáló infrastruktúrájához időkorlátos aláírt jogosultságot kap.

**Genesis** — a protokoll első, kézi barkácsolási fázisa. A localhost (fájl) réteg eszközeivel az aqProtocol és az első identitás DAO-k létrehozása.

**Bogyó flow** — a régi terminológiában a feldolgozási folyamat. Ma kontextusgráf (CTXG).

**Megbízhatóság** (reliability) — funkció DAO mérőszáma, ami mindkét irányba mozoghat. Forrása: protokoll érettség × tulajdonos szabadsági fok. Nem azonos a hitelességgel.

**Korszak-szorzó** (epoch multiplier) — az a szorzó, amelyet egy DAO a létrehozási (vagy publikálási) korszakban kap, és örökre magával viszi. Korábbi korszakban keletkezett DAO szorzója nem módosul visszamenőleg.

**Zárt köri megbízhatóság** — a protokoll érettségi szorzó csökkenő komponense (genesis-től open source-ig). A szűk kör személyes ismertsége mint védelmi réteg.

**Nyílt ismertségi hitelesség** — a protokoll érettségi szorzó növekvő komponense (open source-tól). Külső közreműködők, auditok, közösség által épül.

**Kompromittáltság jelzés** — egy identitás DAO-ra vonatkozó súlyozott figyelmeztetés, amely a jelző hitelességével és szabadsági fokával van súlyozva. Nem automatikus kizárás, hanem vizsgálandó jelölés.

**Recovery visszaigazolás** — egy kompromittált identitás DAO recovery utáni folyamata, amelyben a tulajdonos visszanézi és visszaigazolja (vagy jelölve hagyja) a korábbi kimeneteit.
