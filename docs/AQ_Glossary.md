...
**Referencia** (reference, refs) — egy DAO config-jában a kategorizált, névvel hivatkozható erőforrások (kategóriák: js, css, json, html, others). Két szintű séma: `refs[category][subcategory][name]`. Egy `refs` levél kétféle lehet: lokális ref vagy távoli ref. Egységes (category, sub, name) névtér: egy bejegyzés egyszer szerepel a map-ben. A `refs[cat]`, `refs[cat][sub]`, `refs[cat][sub][name]` kulcsai ref-nevek (engedett: `[a-zA-Z0-9._-]+`, `.` és `..` tiltva). A 2-szintű hivatkozás (`aq.ref` második paramétere, `pages.<key>.<field>`, `exports[].name`) `"sub.name"` formátumú, egzakt 2 szegmens pont-szeparátorral.
...
**tokenId** — funkció DAO azonosítója az aqProtocol szerződésében (PageNFT).
