...
**A protokoll nem kezel identitás DAO-ra vonatkozó kompromittáltság jelzést.** A hibás kimenetek funkció szintű hibajelzéssel kezelendők.

**Hibajelzés (error signal):**
- Identitás DAO jelzi egy funkció DAO hibáját
- Közösségi tagság(ok) nevében, megadva a nézőpontot
- Súly = jelző közösségi DAO-k hitelessége
- Visszavonható a jelző által
- Saját hiba elismerése növeli a hitelességet

**Hasznosság jelzés (utility signal):**
- Identitás DAO jelzi egy funkció DAO hasznosságát
- Közösségi tagság(ok) nevében
- Súly = jelző közösségi DAO-k hitelessége
- Visszavonható a jelző által

**Fizetés:**
- Hibajelzés: jelző közösségi DAO fizeti
- Hasznosság jelzés: alapértelmezetten a jelzett DAO, opcionálisan a jelző
- Visszavonás: a visszavonó fizeti

**Spam védelem**: stake-mentes. A jelzés súlya a választott közösség hitelességéhez kötött. Játékrontó közösség súlytalan.

**Nézőpont**: ugyanaz az identitás különböző tagságok nevében ellentétesen is jelezhet.
...
- Host conf `rpc` mező (string, default Gnosis fallback) — `aqRpcConfig.js`, `aqRpc.js` paraméteres szignatúra
- WEB2 lokális fejlesztői szerver minimum (read-only, 3-port, kézi fájl-elhelyezés)
- **refs/exports/pages szigorú validáció** a DAO config betöltésekor (`validateDaoConfig`, `validateRefsLeaf`, `validateRefName`, `validateRefPath`, `validateDescription`)
- **2-szintű refs séma**: `refs[category][subcategory][name]`. `validateRefName` minden szegmensen, `validateRefPath` a `"sub.name"` formátumon (`aq.ref`, `pages.<key>.<field>`, `exports[].name`).
- **Refs leaf kötelező objektum** (string forma megszűnt): lokális `{cid|path, description}` (description kötelező), távoli `{tokenName}` (description tilos).
- **`validateDescription`**: plain szöveg, `\r\t` tilos, `\n` engedett, ASCII kontroll karakterek tilva, Unicode engedett.
- **`imports` szekció megszüntetve** a config-sémából (beolvad a `refs` távoli levélbe). **`_parent` eltávolítva** a config-sémából. A `_` prefix kivétel nélkül engedett a ref-nevekben.
- **`exports` blokk** (lapos `[{category, name}]` lista, `name` 2-szintű) — séma + eager cross-reference validáció. Runtime enforce nincs (fork-öröklés deklaráció).
- **Távoli ref feloldás** a loaderben: tokenName-alapú, egyszintű, kétlépéses fetch (lásd §2.2 és §6.1 a Guide-ban).
- `aq.ref(category, "sub.name")` async signature; lokális objektum vagy `{cid, cidBase?}` resolved objektum visszatérés. `aq.fetchText` / `aq.fetchBytes` input: lokális objektum, resolved objektum, vagy nyers string (CID/path; tokenId tilos).
- `loadPage` 2-szintű ref feloldás támogatás (`resolveOwnRef`-en keresztül).
- **Protokoll config szekció**: `loader` (top-level, `{cid}` vagy `{path}`) + `gates` (map `{<name>: {tokenId, description}` vagy `{path, description}`}). Nincs `pages`/`defaultPage` a protokoll configban. Runtime nem validált.
- **Új boot flow**: tokens/0 → protokoll config → loader bundle inject. `aqDaoConfig` és `aqPageLoader` host conf mezők megszűntek; új `aqGateDAOName` opcionális.
- **Kapu DAO mint speciális DAO**: `loadGateDao` belépés, közös `_loadDaoConfigInternal` delegálás, `"gate:" + tokenId/path` namespace. Host-szintű renderelés (`<div id="aq-gate-root">`), `window.aqGateInit()` belépési pont.
- **Kapu DAO választás precedencia**: IndexedDB `_protocol/aqGateDAOName` > host conf `aqGateDAOName` > első `gates` kulcs. Hard fail ismeretlen név esetén.
- **`_protocol` namespace** az `aqStorage` modulban: közös `aqStorage*Ns(namespace, ...)` belső függvények + DAO-scoped wrapper-ek + `aqProtocolStorage*` (fix `_protocol` namespace).
- **`window.aqGateApi` expose** (`exposeGateApi`, `tearDownGateApi`): kapu DAO közvetlen API-ja a loader funkcióihoz. `protocolStorage.*` (5 metódus, `_protocol` namespace) és `seed.*` (store, exists). A postMessage `aq.gate.*` capability-k eltávolítva.
- Host conf `openTokenId` mező (kötelező, a megnyitandó tartalmi DAO azonosítója; devmode-ban path engedett, production tokenId)
- **isGate flag eltávolítva**: sem a page sablonban, sem az `AQ_INIT` payloadban nincs `isGate`. Tartalmi DAO sosem fér hozzá a `_protocol` namespace-hez.
- **Contracts map** a DAO config-ban: `{ <name>: { address, rpc?, description } }`. `address` kötelező (`CONTRACT_RE`), `description` kötelező, `rpc` opcionális. Eager validáció betöltéskor.
- **Tokens map** a DAO config-ban: `{ <name>: { contractName, tokenId, description } }`. `contractName` eager check a `contracts` map-en, `tokenId` digits-string, `description` kötelező. Eager validáció betöltéskor.
- **Remote ref tokenName séma**: `{tokenName}` egyetlen mező a leaf-en. A korábbi `{rpc?, contract, tokenId}` hármas eltávolítva.
- **DAO config top-level `rpc` mező**: opcionális string. Fallback prioritás: `contracts.<name>.rpc` > DAO config `rpc` > default Gnosis lista. NEM örökli a host conf-ot.
- **devMode path a remote ref célon**: devMode-ban a remote ref cél leaf `{path, description}` is lehet (production-ban csak `{cid, description}`).
- **Seed store** (`aqSeed` IndexedDB, v1, store: `"seed"`, key: `"current"`): `seedExists()` (csak jelenlét), `seedStore(record)` (felülírás tiltva, `storedAt` auto), `seedGetInternal()` (csak loader belső kód). Kapu DAO: `exists` + `store`; tartalmi DAO: semmi.
- **Seed-gen flow**: boot ellenőrzi `seedExists()` → ha nincs: `loadGateDao(..., "seedGen")`, `window.aqSeedGenComplete()` callback regisztrálva. Ha van: `loadGateDao(...)` + `loadContentDao(openTokenId)`. Biometrikus (WebAuthn PRF) és jelszó (PBKDF2 600k + AES-GCM) ág; BIP-39 12 szó.
- **Konvenciók**: `window.aqGateInit()` (kapu DAO JS belépés, loader hívja), `window.aqSeedGenComplete()` (loader regisztrálja boot során, nincs-seed ágban), `pages.seedGen` (konvencionális pageKey seed-gen flow-ban).
