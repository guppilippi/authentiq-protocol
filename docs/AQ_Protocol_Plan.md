...
A jelenleg implementált mag: `aqBoot.js`, `aqProtocolLoader.js` és a WEB2 szerver réteg (lásd [WEB2 Server Guide](AQ_WEB2_Server_Guide.md)). Minden más ebben a dokumentumban tervként szerepel.
...
A feloldás részletei → lásd [Implementation Guide](AQ_Protocol_Implementation_Guide.md) §6.1.
...
→ Implementált. Lásd [Implementation Guide](AQ_Protocol_Implementation_Guide.md) §2.3.
...
→ Implementált. Lásd [Implementation Guide](AQ_Protocol_Implementation_Guide.md) §5.2–§5.4, §14.
...
A **közösségi identitás DAO**-k minimum tagszáma **függ a korszaktól és a tagok szabadsági fokától**. Az egyéni identitás DAO definíció szerint egyszemélyes — rá ez a minimum nem vonatkozik.

**Alapelvek**:
- 2 tagos közösség nincs (recovery-képtelen)
- Minimum mindig legalább 3 tag (Shamir-szilánk elosztás miatt)
...
### 19.1. Jelenlegi állapot — read-only szerver

→ Implementált. Lásd [WEB2 Server Guide](AQ_WEB2_Server_Guide.md) §1.

### 19.2. Write szerver

→ Implementált. Lásd [WEB2 Server Guide](AQ_WEB2_Server_Guide.md) §2.
